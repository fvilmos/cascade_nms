...
def intersection_over_union(val1,val2):
    ...
    xu = np.min((val1[0],val2[0]))
    yu = np.min((val1[1],val2[1]))

    wu = np.max((val1[0]+val1[2], val2[0]+val2[2])) - xu
    hu = np.max((val1[1]+val1[3], val2[1]+val2[3])) - yu

    # calculate the intersection of boxes
    xi = np.max((val1[0],val2[0]))
    yi = np.max((val1[1],val2[1]))

    wi = np.min((val1[0]+val1[2], val2[0]+val2[2])) - xi
    hi = np.min((val1[1]+val1[3], val2[1]+val2[3])) - yi
...
def non_maximum_surpression(detections,THRESHOLD=0.1, USESORT=False,REVERSE_SORT=True):
    ...
    if len(detections)>0:
        ...
        while len(tdet)>0:
            ...
            for j in range (len(tdet)-1,-1,-1):
                lv = tdet[j]
                iou = intersection_over_union(tst,lv)
