...
if __name__ == '__main__':
    ...
    parser.add_argument('-so', type=bool, help='Sort the detection based on their area, bigest the last', default=True)
    parser.add_argument('-rev', type=bool, help='Sort the detection based on their area, smallest the last', default=True)
    ...
    while True:
        ...
        det = non_maximum_surpression(det, THRESHOLD=args.iou, USESORT=args.so, REVERSE_SORT=args.rev)

        for val in det:
            cv2.rectangle(frame,(val[0],val[1]),(val[0]+val[2],val[1]+val[3]),[0,0,255],3)
